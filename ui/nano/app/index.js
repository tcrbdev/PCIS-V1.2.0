...
const columnsTotalSummary = [{
    ...
    children: [{
        ...
        render: (text, record, index) => (record.Kpi == 'Unit' ? `${parseFloat(text).toFixed(0)}` : record.Kpi == 'Ach' ? `${parseFloat(text).toFixed(parseInt(text) >= 100 ? 0 : 1)}%` : parseFloat(text).toFixed(parseInt(text) >= 100 ? 0 : 1))
    }, {
