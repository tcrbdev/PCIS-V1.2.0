...
import PortfolioChart from '../map/portfolio_chart'
...
class ModalPortfolioChart extends Component {
    state = {
        modalOpen: false,
    }
    ...
    componentDidMount() {
        if (this.props.type) {
            if (document.getElementById('modal-portfolio-chart2') === null || document.getElementById('modal-portfolio-chart2') === undefined) {
                var divModal = document.createElement("div")
                divModal.id = 'modal-portfolio-chart2'

                document.getElementById('add-portfolio-chart2').appendChild(divModal)
            }
        }
        else {
            if (document.getElementById('modal-portfolio-chart') === null || document.getElementById('modal-portfolio-chart') === undefined) {
                var divModal = document.createElement("div")
                divModal.id = 'modal-portfolio-chart'

                document.getElementById('add-portfolio-chart').appendChild(divModal)
            }
        }
    }
        ...
        return (
            <div style={{ marginLeft: '0px' }}>
                <Modal
                    ...
                    maskClosable={false}
                    mask={false}
                    getContainer={() => this.props.type ? document.getElementById('modal-portfolio-chart2') : document.getElementById('modal-portfolio-chart')}
                >
                ...
                <Tooltip title="Portfolio Quality" placement="topRight"><FontAwesome style={{ color: '#64a717' }} name="dollar" onClick={this.handleModal} /></Tooltip>
            </div>
        )
    }
}
...
export default connect(
    (state) => ({
        RELATED_CHART_PORTFOLIO_QUALITY: state.RELATED_CHART_PORTFOLIO_QUALITY,
        CHART_PORTFOLIO_QUALITY_BY_CA: state.CHART_PORTFOLIO_QUALITY_BY_CA
    }),
    {
    })(ModalPortfolioChart)